from model import get_current_command_state, set_current_command_state, get_persons_str, get_persons, get_owe_summary, set_persons, get_pool_summary_str
...
def init_command_handlers():
    ...
    handlers.append(CommandHandler('pool', pool_command))
    handlers.append(CommandHandler('log', log_command))
...
async def pool_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if get_current_command_state() == None:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=get_pool_summary_str(get_persons()))
